...
#'
#' @seealso
#' \code{vignette("fit_bayesian_growth")} for usage examples.
#'
#' \code{vignette("partial_pooling")} for hierarchical modeling of imbalanced sex ratios.
#'
#' \href{../doc/Understanding_vitalBayes.html#growth}{Statistical Methods: Growth Models}
#' for equations and model comparison.
#'
#' \href{../doc/Understanding_vitalBayes.html#maturity-param}{Statistical Methods: Maturity-Based Parameterization}
#' for the derivation of k from maturity parameters.
#'
#' \href{../doc/Understanding_vitalBayes.html#linf}{Statistical Methods: L-infinity Constraint}
...
fit_bayesian_growth <- function(
  ...
  if (!is.null(birth_stanfit)) {
    L0_prior_info <- .extract_birth_prior(birth_stanfit, multiplier = 1.5)
    mean_L0_nat <- rep(L0_prior_info$natural["mean"], n_sex)
    sd_L0_nat <- rep(L0_prior_info$natural["sd"], n_sex)
  ...
  if (!k_based) {
    if (!is.null(length.mature_stanfit)) {
      Lmat_info <- .extract_maturity_prior(length.mature_stanfit, "L50", multiplier = 1.5)
      mean_Lmat_nat <- sapply(Lmat_info, function(x) x$natural["mean"])
      sd_Lmat_nat <- sapply(Lmat_info, function(x) x$natural["sd"])
      if (length(mean_Lmat_nat) < n_sex) {
        mean_Lmat_nat <- rep(mean_Lmat_nat, n_sex)
        sd_Lmat_nat <- rep(sd_Lmat_nat, n_sex)
      }
    ...
    if (!is.null(age.mature_stanfit)) {
      tmat_info <- .extract_maturity_prior(age.mature_stanfit, "t50", multiplier = 1.5)
      mean_tmat_nat <- sapply(tmat_info, function(x) x$natural["mean"])
      sd_tmat_nat <- sapply(tmat_info, function(x) x$natural["sd"])
      if (length(mean_tmat_nat) < n_sex) {
        mean_tmat_nat <- rep(mean_tmat_nat, n_sex)
        sd_tmat_nat <- rep(sd_tmat_nat, n_sex)
      }
    ...
    if (k_based) {
      if (is_twosex) {
        if (use_pooling) {
          ...
          list(
            log_Linf_direct = log(mean_Linf_nat),
            log_L0_direct   = log(mean_L0_nat),
            log_k_direct    = log(k_init),
            sigma           = c(0.1, 0.1)
          )
        }
      } else {
      ...
      if (is_twosex) {
        if (use_pooling) {
          ...
          list(
            log_Linf_direct = log(mean_Linf_nat),
            log_L0_direct   = log(mean_L0_nat),
            log_Lmat_direct = log(mean_Lmat_nat),
            log_tmat_direct = log(mean_tmat_nat),
            sigma           = c(0.1, 0.1)
          )
        }
      } else {
