---
title: "Modeling Individual Growth with fit_bayesian_growth()"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Modeling Individual Growth with fit_bayesian_growth()}
...
The CV approach ensures priors scale sensibly across species of different sizes --- a 20% CV on $L_\infty$ means the same thing whether the species is 50 cm or 300 cm.
...
| $L_\infty$ boundary pile-up (posterior median at $L_{max}$) | Consider wider `CV_Linf` or different growth model |
...
- `vignette("chen_watanabe_reparameterization")` --- How growth parameters feed into CW mortality estimation
- `vignette("mortality_estimation")` --- Natural mortality from growth posteriors
- `vignette("model_diagnostics")` --- LOO-CV model comparison and convergence checks
